    ...
    <section id="product" ref={ref} style={{ padding: isMobile ? '0 16px 80px' : '0 24px 120px', opacity: visible ? 1 : 0, transform: visible ? 'translateY(0)' : 'translateY(24px)', transition: 'all 0.7s ease' }}>
      <style>
        {`
          ...
          .floating-tooltip {
            ...
            pointer-events: none;
            opacity: 0;
            animation: fadeInFloat 1s ease forwards 1.5s;
          }
      ...
      <div style={{ maxWidth: '1140px', margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: isMobile ? '40px' : '56px' }}>
          ...
          <h2 style={{ fontSize: 'clamp(28px, 4vw, 40px)', fontWeight: '600', letterSpacing: '-0.025em', color: '#f5f5f5', margin: 0, marginBottom: '16px' }}>
            A dashboard built for clarity
          </h2>
          <p style={{ fontSize: '16px', color: '#888', maxWidth: '500px', margin: '0 auto', lineHeight: '1.6' }}>
            Get real-time insights into your API endpoints and websites without the clutter. Everything you need, exactly where you expect it.
          </p>
        </div>
        ...
        <div style={{
          ...
          <div style={{ padding: isMobile ? '16px' : '24px', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? '16px' : '20px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                {sites.map((site, i) => (
                  <div key={i} style={{
                    ...
                    {site.status === 'DOWN' && !isMobile && (
                      <div className="floating-tooltip" style={{ right: '-12px', top: '-12px', backgroundColor: '#ef4444', color: '#fff', boxShadow: '0 4px 12px rgba(239, 68, 68, 0.3)' }}>
                        Real-time WebSocket Alerts
                      </div>
                    )}
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      <span className={site.status === 'UP' ? 'animate-pulse-dot' : ''} style={{ width: '6px', height: '6px', borderRadius: '50%', backgroundColor: site.color, flexShrink: 0, boxShadow: `0 0 6px ${site.color}60` }} />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {/* Uptime metric */}
              <div style={{ backgroundColor: '#0f0f0f', borderRadius: '10px', border: '1px solid #1a1a1a', padding: '20px 24px', position: 'relative' }}>
                {!isMobile && (
                  <div className="floating-tooltip" style={{ right: '16px', top: '-10px' }}>
                    Instant Analytics
                  </div>
                )}
